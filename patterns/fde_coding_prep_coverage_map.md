...
| 17 | 🟠 | String frequency / character counting | Given log labels, event types, or tokens, count frequencies and compare/group strings | Covered: Equvivalent services - char freq.  Covers anagrams, first non-repeating char, frequency comparison, token counts. Core structures: dict / Counter / sorted key. |
| 18 | 🟠 | Substring / sliding-window strings | Find longest valid substring, recent unique users/tokens, or events satisfying window constraints | Covered: Sliding Window / Two Pointers. High-priority LeetCode pattern. Core structures: two pointers, set/dict counts, left/right window. |
...
| 20 | 🟠 | Binary search / first anomaly | Find first failing version/timestamp, first unhealthy state, threshold crossing, rotated sorted array basics | Covered: First Unhlealthy Version - binary search. Important weak flank; do not over-invest, but know the boundary-search skeleton. |
| 21 | 🟠 | Stack for strings | Validate nested syntax, simplify paths, remove adjacent duplicates, process undo-like event streams | Covered: stack for Normalize Path, Near Palindrome. Medium priority. Common SWE pattern; possible string-heavy prompt. |
| 22 | 🟠 | String graph construction | Build graph from string pairs/relationships and run BFS/DFS/topo/cycle detection | Covered: Impacted services - graph from strings, DFS/BFS. Very high relevance: bridge between LeetCode strings and SRE/distributed systems. |
